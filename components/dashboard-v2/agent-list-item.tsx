  ...
  return (
    <button
      ...
      className={cn(
        ...
        isSelected ? "bg-zinc-800 text-zinc-50" : "text-zinc-400 hover:bg-zinc-800/50 hover:text-zinc-50"
      )}
    >
      <div className="relative shrink-0">
        ...
        <span className={cn("absolute bottom-0 right-0 h-2.5 w-2.5 rounded-full border-2 border-zinc-900", dotColor)} />
      </div>
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm font-medium">{name}</p>
        <p className="truncate text-xs text-zinc-500">{roleLabels[role] ?? role}</p>
      </div>
    </button>
  );
}

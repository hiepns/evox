        ...
        <div key={i} className="flex-1 flex flex-col items-center gap-1">
          <div
            className={cn(
              ...
              d.value > 0 ? "bg-red-500/60" : "bg-[#222222]"
            )}
            style={{ height: `${(d.value / max) * 100}%`, minHeight: d.value > 0 ? "4px" : "2px" }}
          />
          <span className="text-[8px] text-[#555555]">{d.label}</span>
        </div>
      ))}
    </div>
  );
    ...
    <div className={cn("p-6 space-y-6 overflow-auto", className)}>
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="rounded-lg border border-[#222222] bg-[#111111] p-4">
          <div className="text-xs text-[#666666] mb-1">Success Rate (24h)</div>
          <div className={cn(
            "text-2xl font-bold",
        ...
        {/* Tasks Completed */}
        <div className="rounded-lg border border-[#222222] bg-[#111111] p-4">
          <div className="text-xs text-[#666666] mb-1">Completed (7d)</div>
          <div className="text-2xl font-bold text-white">{metrics.completed7d}</div>
          <div className="mt-2 flex items-center gap-2">
            <span className="text-xs text-[#555555]">24h:</span>
            <span className="text-xs text-[#888888]">{metrics.completed24h}</span>
            <span className="text-xs text-[#555555]">30d:</span>
            <span className="text-xs text-[#888888]">{metrics.completed30d}</span>
          </div>
        </div>

        {/* Errors */}
        <div className="rounded-lg border border-[#222222] bg-[#111111] p-4">
          <div className="text-xs text-[#666666] mb-1">Errors (7d)</div>
          <div className={cn(
            "text-2xl font-bold",
          ...
          <div className="mt-2 flex items-center gap-2">
            <span className="text-xs text-[#555555]">24h:</span>
            <span className={cn("text-xs", metrics.errors24h > 0 ? "text-red-400" : "text-[#888888]")}>
              {metrics.errors24h}
            </span>
          </div>
        </div>
        ...
        {/* MTTR */}
        <div className="rounded-lg border border-[#222222] bg-[#111111] p-4">
          <div className="text-xs text-[#666666] mb-1">MTTR</div>
      ...
      {/* Error Trend Chart */}
      <div className="rounded-lg border border-[#222222] bg-[#111111] p-4">
        <div className="text-xs text-[#666666] mb-3">Error Trend (7 days)</div>
        <ErrorBars data={metrics.errorsByDay} />
      </div>

      {/* Agent Health Grid */}
      <div>
        <div className="text-xs text-[#666666] mb-3 uppercase tracking-wider">Agent Health</div>
              ...
              >
                ...
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-[#555555] capitalize">{agent.status}</div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div>
                    <div className="text-[#555555]">Completed</div>
                    <div className="text-white font-medium">{agent.tasksCompleted}</div>
                  </div>
                  <div>
                    <div className="text-[#555555]">Success Rate</div>
                    <div className={cn(
                      "font-medium",
                ...
                <div className="mt-3 pt-3 border-t border-[#222222]">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] text-[#555555]">7-day trend</span>
                    <Sparkline

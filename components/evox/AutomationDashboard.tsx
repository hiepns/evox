...
export function AutomationDashboard({ className }: AutomationDashboardProps) {
  ...
  const chipBase =
    "rounded-lg border border-[#222222] bg-[#111111] p-4 transition-colors";
  ...
  return (
    <div className={cn("p-6 space-y-6 overflow-auto", className)}>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1 rounded-lg bg-[#111111] border border-[#222222] p-1">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className={chipBase}>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-medium uppercase tracking-wider text-[#666666]">Success</span>
          </div>
          <div className="text-3xl font-bold text-emerald-400">{stats.success}</div>
          <div className="text-sm text-[#555555]">{stats.successPercent}%</div>
        </div>
        ...
        <div className={chipBase}>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-medium uppercase tracking-wider text-[#666666]">Retries</span>
          </div>
          <div className="text-3xl font-bold text-yellow-400">{stats.retries}</div>
          <div className="text-sm text-[#555555]">{stats.retriesPercent}%</div>
        </div>
        ...
        <div className={chipBase}>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-medium uppercase tracking-wider text-[#666666]">Blocked</span>
          </div>
          <div className="text-3xl font-bold text-red-400">{stats.blocked}</div>
          <div className="text-sm text-[#555555]">{stats.blockedPercent}%</div>
        </div>
      </div>
      ...
      <div className={cn(chipBase, "space-y-3")}>
        ...
        {pendingApprovals && pendingApprovals.length > 0 ? (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className="ml-2 text-xs text-[#555555]">
          ...
          <div className="text-sm text-[#555555] py-2">No pending approvals</div>
        )}
      </div>
            ...
            return (
              ...
              >
                ...
                <span
                  className={cn(
                    "text-xs",
                    isImminent ? "text-emerald-400" : "text-[#666666]"
                  )}
                >
                  Next in {nextRun}
                </span>
      ...
      <div className={cn(chipBase, "space-y-3")}>
        ...
        {recentFailures.length > 0 ? (
          ...
          <div className="text-sm text-[#555555] py-2">No recent failures</div>
        )}
      </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {cronStatus.agentHeartbeats.map((agent: AgentHeartbeat) => {
            ...
            return (
              <div
                key={agent.name}
                className="flex items-center gap-3 rounded-md bg-[#0a0a0a] px-3 py-2"
              >
                <span
                  className={cn(
                    "h-2 w-2 rounded-full",
                    isRecent ? "bg-emerald-400" : "bg-[#333333]"
                  )}
                />
                <div>

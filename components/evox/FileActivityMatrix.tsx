...
export function FileActivityMatrix({ className, limit = 50 }: FileActivityMatrixProps) {
  ...
  if (!activities || activities.length === 0) {
    return (
      <div className={cn("flex flex-col border border-[#222222] bg-[#111111] rounded-lg", className)}>
        ...
        <div className="flex-1 flex items-center justify-center py-8 text-xs text-[#555555]">
  ...
  return (
    <div className={cn("flex flex-col border border-[#222222] bg-[#111111] rounded-lg", className)}>
      ...
      {hoveredActivity && (
        <div className="border-t border-[#222222] px-3 py-2">
          <div className="flex items-center gap-2 text-xs">
            ...
            <span className="font-mono text-[#fafafa]">{hoveredActivity.filePath}</span>
            <span className="text-[#555555]">by</span>
            <span className="font-medium text-[#fafafa]">{hoveredActivity.agentName.toUpperCase()}</span>
      ...
      {/* Legend */}
      <div className="flex items-center gap-4 border-t border-[#222222] px-3 py-2">
        {Object.entries(actionColors).map(([action, colors]) => (
          <div key={action} className="flex items-center gap-1.5">
            <span className={cn("h-2 w-2 rounded-full", colors.dot)} />
            <span className="text-[10px] text-[#555555] capitalize">{action}</span>
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="space-y-3">
      {Array.from(folderGroups.entries()).map(([folder, files]) => (
        <div key={folder} className="flex items-start gap-3">
          {/* Folder name */}
          <div className="w-24 shrink-0 truncate text-xs text-[#888888]">
            {folder}/
          </div>
          ...
          <div className="ml-4 mt-1 space-y-1">
            {files.map((file, idx) => {
              ...
              return (
                ...
                >
                  <span className="text-[#555555]">{isLast ? "└─" : "├─"}</span>
                  <span className={cn(
                  ...
                  <span className={cn(
                    "truncate",
                    recent ? "text-[#fafafa]" : "text-[#888888]"
                  )}>
                    {fileName}
                  </span>
